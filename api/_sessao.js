...
// Arquivos com _ não viram rota na Vercel
...
// Em localhost (http), o navegador descarta cookie "Secure".
...
export function criarCookie(req) {
  const expira =
    Math.floor(Date.now() / 1000) + DURACAO;

  const token =
    expira + '.' + assinar(String(expira));
...
export function codigoConfere(digitado) {
  const certo =
    (process.env.CODIGO_ACESSO || '').trim();

  if (
    !certo ||
    typeof digitado !== 'string'
  ) {
    return false;
  }
